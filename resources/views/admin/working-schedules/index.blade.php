        ...
        <div>
            <div>
                @if(auth()->user()->isAdmin())
                    <form action="{{ route('admin.working-schedules.delete-all') }}" method="POST" class="d-inline"
            ...
            <form method="GET" action="{{ route('admin.working-schedules.index') }}" class="form-row">
                <div class="form-group col-md-4">
                    ...
                    <input type="text" name="employee_name" id="employee_name" class="form-control"
                        placeholder="Nhập tên nhân viên" value="{{ $filters['employee_name'] ?? '' }}">
                </div>
                <div class="form-group col-md-4">
                ...
                <div class="form-group col-md-4 d-flex align-items-end">
                    ...
                    <a href="{{ route('admin.working-schedules.index') }}" class="btn btn-secondary"><i
                            class="fas fa-redo"></i> Làm mới</a>
                    @if(auth()->user()->isAdmin())
                        <a href="{{ route('admin.working-schedules.trash') }}" class="btn btn-warning ml-2"><i
                                ...
                                @foreach($dateGroups as $group)
                                    ...
                                    <tr class="schedule-row">
                                        <td class="align-middle text-center">
                                            ...
                                            @if($firstSchedule)
                                                @php
                                                    $canViewDetail = auth()->user()->isAdmin();
                                                    if (!$canViewDetail && auth()->user()->isEmployee()) {
                                                        $currentUserId = auth()->id();
                                                        $canViewDetail = ($firstSchedule->employee && $firstSchedule->employee->user_id == $currentUserId);
                                                    }
                                                @endphp

                                                @if($canViewDetail || auth()->user()->isAdmin())
                                                    <a href="{{ route('admin.working-schedules.show', $firstSchedule->id) }}"
                                                        class="btn btn-sm btn-info btn-block mt-2" title="Xem chi tiết">
                                                ...
                                                @if($schedule)
                                                    ...
                                                    <div class="employee-info">
                                                        ...
                                                        <div class="employee-actions">
                                                            @if(auth()->user()->isAdmin() || (auth()->user()->isEmployee() && $user && $user->id === auth()->id()))
                                                                {{-- <a href="{{ route('admin.working-schedules.show', $schedule->id) }}"
